...
The target implements a simple client-server service. Only one client per time is supported. The client does send messages and based on their contents the server does execute some parts of codes. 
...
Before going further by showing the actual PoC, more information are provided below.

(1) The body request sent from the client is saved at the address `0x10000000`, which is always initialezed before the recv call with the constants `0x5050505050505050` and `0x0CF58585858585858` repeated to fill completely the memory area. Also each byte `"0x33"` and `"0x2B"` inside the body is set to zero. 

(2) Finally, before doing the arbitrary assembly code the program does change the address target by adding the return value of the `recv` which if everything went well should be the bytes received before. So at least we can choose which instruction, out of the constants 
`0x5050505050505050` and `0x0CF58585858585858`, to execute. Below is shown the disassembly of those constants.
